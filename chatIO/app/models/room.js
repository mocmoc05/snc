...
var remove = function(data, callback){
	roomModel.remove(data, callback);
}
...
var getUserByRoomId = function(roomId, callback){
	roomModel.findById(roomId, function(err, room){
		var us=[];
		room.users.forEach(element => {
			User.find({username: element}, function(err, user){
				us.push(user);
			})
		});
		callback(us);
...
var getUserByUsername = function(username, callback){
	roomModel.find({ users: { $elemMatch: { username:username }} }, function(err, rooms){
		callback(rooms);
	})
}
/**
