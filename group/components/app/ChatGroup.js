...
export default class chat extends Component {
    ...
    constructor(props) {
        ...
        this.receiver= state.params.listUser;
        this.socket=state.params.socket;

        console.log("chat nhan",state.params)
        console.log("chat da nhan danh sach userOn",this.receiver,this.socket);
        console.log("username:",typeof(this.receiver),this.receiver)
        ...
        this.socket.on('msg-group', (message) => {
    ...
    componentDidMount(){
        ...
        .then((value) => {
            ...
            this.socket.emit("get-history-group", {
        ...
        this.socket.on('send-history-group', (data) => {
    ...
    onSend(messages = []) {
        ...
        .then((res)=>{
            var profile = JSON.parse(res);
            this.socket.emit('send-group', {roomId:this.roomId,message:objMessage, receiver: this.receiver, emitter: profile.user.username});
        })
    }
    ...
    render() {
        return (
            <View style={styles.chatWrap}>
                <View style={{height:60,flexDirection:"row",alignItems:"center",borderBottomWidth:1,borderBottomColor:"gray"}}>
                <TouchableOpacity
                    ...
                    <Text style={{fontSize:20,color:"black",paddingLeft:90,fontFamily:"Cochin"}}>Tin Nhắn</Text>
                </View>
                <GiftedChat
                    style={{borderWidth:1}}
